package edu.wgu.cristianreyes.application.db;
...
public class ConnectionPool {
     ...
     * @throws ConnectionsUnavailableException if there is no available connection in the connection pool.
     */
    public Connection getConnection() throws ConnectionsUnavailableException{
        ...
        if (connectionPool.size() > 0) {
            ...
            try {
                if (!conn.isValid(0)) {
                    refreshConnections();
                    throw new ConnectionsUnavailableException("Connections expired. Connections refreshed. Try again");
                } else {
    ...
    private void refreshConnections() {
        ArrayList<Connection> connectionsList = new ArrayList<>(connectionPool);
        ...
        for (Connection connection : connectionsList) {
            try {
                if (!connection.isValid(0)) {
                    ...
                    connectionPool.add(createConnection(connectionUrl, connectionUsername, connectionPassword));
                }
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
